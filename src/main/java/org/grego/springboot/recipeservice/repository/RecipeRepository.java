...
public class RecipeRepository {
    ...
    public Flux<Recipe> findAllByName(String name) {
        return template.select(Recipe.class).from(Recipe.RECIPES_TABLE_NAME).matching(query(where(Recipe.NAME_COLUMN_NAME).is(name))).all();
    }

    public Flux<Recipe> findHighestVariationByName(String name) {
        return template.select(Recipe.class).from(Recipe.RECIPES_TABLE_NAME).matching(query(where(Recipe.NAME_COLUMN_NAME).is(name))).all();
    }

    public Flux<Recipe> findAll() {
        return template.select(Recipe.class).from(Recipe.RECIPES_TABLE_NAME).all();
    }

    public Mono<Boolean> existsById(long recipeId) {
        return template.select(Recipe.class).from(Recipe.RECIPES_TABLE_NAME).matching(query(where(Recipe.RECIPE_ID_COLUMN_NAME).is(recipeId))).exists();
    }

    public Mono<Recipe> findById(long recipeId) {
        return template.select(Recipe.class).from(Recipe.RECIPES_TABLE_NAME).matching(query(where(Recipe.RECIPE_ID_COLUMN_NAME).is(recipeId))).one();
    }

    public Mono<Recipe> save(Recipe recipe) {
        return template.insert(Recipe.class)
    ...
    public Mono<Long> deleteById(long recipeId) {
        return template.delete(Recipe.class)
